...
rem Ayara, non-interactive nhentai.net manga downloader
...
set MAIN_JAR=ayara-%PROJECT_VERSION%.jar
...
rem ------ jsoup dependency --------------------------------------------------

set JSOUP_JAR=target\installer\input\libs\jsoup-1.13.1.jar

rem generate jsoup module

"%JAVA_HOME%\bin\jdeps" --generate-module-info target\mods %JSOUP_JAR%
set jsoup_mods=target\mods\org.jsoup\

rem build jsoup module
...
copy COPYING target\installer\ayara-%PROJECT_VERSION%\
copy CHANGELOG.md target\installer\ayara-%PROJECT_VERSION%\
xcopy /S /Q doc\* target\installer\ayara-%PROJECT_VERSION%\

echo compress the package
cd target\installer
jar -cfM ayara-%PROJECT_VERSION%-bin.zip ayara-%PROJECT_VERSION%
cd ..\..\